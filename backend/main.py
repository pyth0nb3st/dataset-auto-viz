...
from app.agentlib.skills import install_packages, dataset_glance
from app.agentlib.agents import create_image_to_text_agent, create_visual_plan_agent, create_plot_agent, create_data_analysis_report_agent
...
def analyze_dataset(workspace: str, language: str, file: UploadFile = File(...)):
    workspace_path = settings.WORKSPACE_DIR / workspace
    plan_agent = create_visual_plan_agent(quasar_alpha, workspace=workspace_path, output_callback=token_callback, functions=[dataset_glance])
    plot_agent = create_plot_agent(quasar_alpha, workspace=workspace_path, output_callback=token_callback)
    image_to_text_agent = create_image_to_text_agent(gemini_2_0_flash, language, output_callback=token_callback)
    data_analysis_report_agent = create_data_analysis_report_agent(quasar_alpha, language, workspace=workspace_path, output_callback=token_callback)
    ...
    try:
        ...
        # Analyze dataset
        result = plan_agent.user_input(f"{file_path} 看一下这个数据集，只有最后一行的变量会被返回，不要做异常处理")
        ...
        for path in visualization_paths:
            ...
            url = f"http://localhost:8000/static/{relative_path}"
            description = image_to_text_agent.run([
                {
                    'image': path,
                    'text': f'生成给定图片的描述, using {language}'
                }
            ])
        ...
        try:
            ...
            data_analysis_report_url = f"http://localhost:8000/static/{Path(data_analysis_report).relative_to(settings.STATIC_DIR)}"
        except Exception as e:
        ...
        return JSONResponse(content={
            ...
            "data_analysis_report": data_analysis_report_url if data_analysis_report_url else str(data_analysis_report)
        })
    except Exception as e:
        return JSONResponse(
            status_code=500,
